        ...
        <div className="grid md:grid-cols-2 gap-8">
          {services.map((service, i) => (
            <FadeIn key={i} delay={i * 0.1}>
              <div className="group bg-white rounded-3xl p-8 shadow-sm border border-slate-100 hover:shadow-lg hover:-translate-y-1 hover:border-electric/20 transition-all duration-300 h-full">
                <div className="w-16 h-16 rounded-2xl bg-electric/10 flex items-center justify-center mb-6 group-hover:bg-electric transition-colors duration-300">
                  <div className="[&>svg]:text-electric group-hover:[&>svg]:text-white [&>svg]:transition-colors">
                    {service.icon}
                  </div>
                </div>
                <h3 className="text-2xl font-bold text-navy-950 mb-3">{service.title}</h3>
                <p className="text-navy-900/70 mb-8 leading-relaxed">{service.desc}</p>
