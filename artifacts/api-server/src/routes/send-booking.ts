...
function buildInternalNotificationHtml(data: BookingRequestBody): string {
  return `
...
<head>
  ...
  <title>Ihre Terminbestätigung – CleanWeb Agency</title>
  <!--[if mso]>
  <style type="text/css">
              ...
              <table border="0" cellpadding="0" cellspacing="0" width="100%">
                ...
                <tr>
                  <td style="color: #9ca3af; font-size: 16px; line-height: 24px; text-align: center; padding-bottom: 32px;">
                    Hallo ${escapeHtml(firstName)}, vielen Dank für Ihre Terminbuchung. Wir freuen uns auf das Gespräch mit Ihnen und melden uns zum vereinbarten Zeitpunkt.
                  </td>
                </tr>
                    ...
                    <table border="0" cellpadding="0" cellspacing="0" width="100%" style="background-color: #030712; border-radius: 8px;">
                      ...
                      <tr>
                        <td style="padding: 16px;">
                          ...
                          <span style="color: #e5e7eb; font-size: 15px;">Kostenlose Beratung · 30 Min · Online oder Telefon</span>
                        </td>
                      </tr>
                    </table>
                  </td>
                    ...
                    <table border="0" cellpadding="0" cellspacing="0" width="100%" style="background-color: rgba(37, 99, 235, 0.08); border: 1px solid rgba(37, 99, 235, 0.15); border-radius: 8px;">
                      <tr>
                        <td style="padding: 16px; color: #93c5fd; font-size: 14px; line-height: 22px;">
                          Bei Rückfragen oder falls Sie den Termin verschieben möchten, antworten Sie einfach auf diese E-Mail oder kontaktieren Sie uns direkt.
                        </td>
                      </tr>
                    </table>
                  </td>
              ...
              <table border="0" cellpadding="0" cellspacing="0" width="100%" style="border-top: 1px solid #1f2937; padding-top: 30px;">
                <tr>
                  <td align="center" style="color: #4b5563; font-size: 12px; line-height: 18px;">
                    ...
                    <a href="mailto:info@cleanwebag.de" style="color: #4b5563; text-decoration: none;">info@cleanwebag.de</a> · 
...
function buildConfirmationPlainText(data: BookingRequestBody): string {
  const firstName = data.name.split(" ")[0];

  return `
Hallo ${firstName},

vielen Dank für Ihre Terminbuchung bei der CleanWeb Agency. Wir freuen uns auf das Gespräch mit Ihnen.

Ihre Termindetails:
- Datum: ${formatDateGerman(data.date)}
- Uhrzeit: ${data.time} Uhr
- Art: Kostenlose Beratung · 30 Min · Online oder Telefon
...
router.post("/send-booking", async (req: Request, res: Response) => {
  try {
    if (!validateBody(req.body)) {
      res.status(400).json({ error: "Ungültige Buchungsdaten. Bitte füllen Sie alle Pflichtfelder aus." });
      return;
    }
    ...
    const internalHtml = buildInternalNotificationHtml(req.body);
    ...
    const internalResponse = await fetch("https://api.brevo.com/v3/smtp/email", {
      ...
      body: JSON.stringify({
        ...
        subject: `Neue Terminbuchung: ${formatDateGerman(req.body.date)} um ${req.body.time} Uhr`,
    ...
    if (!internalResponse.ok) {
      ...
      logger.error({ status: internalResponse.status, body: errorBody }, "Brevo API error (booking internal notification)");
      res.status(502).json({ error: "Buchung konnte nicht verarbeitet werden. Bitte versuchen Sie es später erneut." });
      return;
    }
    ...
    try {
      ...
      if (!confirmationResponse.ok) {
        ...
        logger.error({ status: confirmationResponse.status, body: errorBody }, "Brevo API error (booking customer confirmation)");
      } else {
        logger.info({ email: req.body.email }, "Booking confirmation email sent");
      }
    } catch (confErr) {
      logger.error(confErr, "Failed to send booking confirmation email");
    }

    logger.info(
      { email: req.body.email, name: req.body.name, date: req.body.date, time: req.body.time },
      "Booking email sequence completed",
    );

    res.json({ success: true });
  } catch (err) {
    logger.error(err, "Failed to process booking submission");
